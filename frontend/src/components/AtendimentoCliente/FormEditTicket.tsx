...
import { ContactType, Ticket, TicketType } from "@/lib/types";
...
export default function FormEditTicket({
  ...
  const {
    ...
    defaultValues: {
      passiveContact: String(ticket?.passiveContact),
      contactType: ticket?.contactType,
      deadline: formatDateToInput(ticket?.deadline) || "",
  ...
  const [searchTerm, setSearchTerm] = useState(watch("reason") || "");
  ...
  const onSubmit = async (data: TicketSchema) => {
    ...
    const response = await updateTicket(data.id as string, dataToSend);

    if(response) {
    toast.success("Ticket editado com sucesso!");
    onClose();
    } else {
      toast.error("Erro ao editar o ticket");
    }
  };
  ...
  return (
    <div className='pb-8'>
      <div className='p-6'>
        ...
        <h2 className='text-xl font-semibold mb-4'>Ticket #{watch("id")}</h2>
